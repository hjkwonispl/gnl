...
def main(exp_args, device):
  ...
  # Output dir
  exp_time_stamp = datetime.today().strftime('%y%m%d_%H%M%S')
  exp_output_dir = '{}_{}'.format(
    exp_args.model_name,
    exp_time_stamp,
  )
  exp_output_dir = os.path.join('output', exp_output_dir)
  ...
  imagenet = load_db.ImageNet(
    val_dir=os.path.join(exp_args.data_dir, 'imagenet2012'),
    device=device,
  )
  ...
  for i, data_idx in enumerate(
    ...
    # Save results.
    result_name = imagenet.val_img_name_template + '.' + exp_args.method
    ...
    draw_deletion_score_graph(del_score, exp_output_dir, result_name)
    draw_insertion_score_graph(ins_score, exp_output_dir, result_name)
